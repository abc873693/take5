...
- **半天請假** → 仍打卡，但落在請假時段的那張卡跳過：
  - 下午請假（如 14:00–18:00）→ 上班卡照打、**下班卡跳過**
  - 上午請假（如 09:00–13:00）→ **上班卡跳過**、下班卡照打
  - ⚠️ 請假交界時間的卡（例如下午請假的 13:00 下班卡）落在固定 cron 時間之外，**需自己手動補打**
- 加 `--force` 或 `CLOCK_FORCE=1` 可無視以上強制打卡

> GitHub Actions 的 cron 改為**每天**觸發（09:00 上班、18:30 下班），實際打不打由腳本判斷，
> 所以假日加班、半天假都能正確處理。請假若仍在審核中也會被視為要請（避免誤打），
